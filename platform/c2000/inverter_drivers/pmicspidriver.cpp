...
void PmicSpiDriver::InitGPIOs() {
    ...
    // SPISOMIA.
    InitGPIO(DEVICE_TESLAM3_GPIO_PIN_PMIC_MISO, DEVICE_TESLAM3_GPIO_CFG_PMIC_MISO);

    // SPISIMOA
    InitGPIO(DEVICE_TESLAM3_GPIO_PIN_PMIC_MOSI, DEVICE_TESLAM3_GPIO_CFG_PMIC_MOSI);

    // SPISTEA / CS
    InitGPIO(DEVICE_TESLAM3_GPIO_PIN_PMIC_CS, DEVICE_TESLAM3_GPIO_CFG_PMIC_CS);

    // SPICLK.
    InitGPIO(DEVICE_TESLAM3_GPIO_PIN_PMIC_CLK, DEVICE_TESLAM3_GPIO_CFG_PMIC_CLK);
}
...
void PmicSpiDriver::InitSPIPort()
{
    ...
    SPI_setConfig(m_base, DEVICE_LSPCLK_FREQ, SPI_PROT_POL0PHA0,
...
void PmicSpiDriver::WriteData(uint16_t data)
{
    SPI_writeDataNonBlocking(m_base, data);
}

uint16_t PmicSpiDriver::ReadData()
{
    return SPI_readDataBlockingNonFIFO(m_base);
}
